...
const StaffCard = ({ user, refetch }) => {
  ...
  const updateBlock = async () => {
    try {
      if (deleteLoading || blockLoading) return;
      ...
      <div className="relative flex justify-center items-center">
        ...
        {option && (
          ...
          >
            ...
            <button
              className={` py-2 px-4 text-red-700 hover:bg-neutral-100 transition-colors ${(deleteLoading || blockLoading) && "cursor-not-allowed opacity-70"}`}
              onClick={deleteUser}
              disabled={deleteLoading || blockLoading}
            >
              {deleteLoading ? (
